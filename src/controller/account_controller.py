...
class AccountController:
    def __init__(self) -> None:
        self._repository = AccountDatabaseRepository(db=DatabaseProvider().get_connection())
        self._presenter = BasePresenter()

    def create_account(self, account: Account):
        use_case = Sigin(account_repository=self._repository)
        try:
    ...
    def login(self, login: LoginRequest):
        use_case = Login(account_repository=self._repository)
        return self._presenter.exception_handler(method=use_case.run, params=[login.email, login.password])

    def get_account_by_id(self, id: str):
        use_case = AccountGet(account_repository=self._repository)
