...
def test_status_not_allowed(ride_repository, ride_in_progress):
    with pytest.raises(ErrorStatusNotAllowed):
        use_case = RidePayment(ride_repository=ride_repository)
        use_case.run(ride_id=ride_in_progress.ride_id, credit_card_token="in_progress", amount="in_progress")


def test_success_payment(ride_repository, ride_pending_pay):
    use_case = RidePayment(ride_repository=ride_repository)
    ride = use_case.run(ride_id=ride_pending_pay.ride_id, credit_card_token="Success", amount="Success")
